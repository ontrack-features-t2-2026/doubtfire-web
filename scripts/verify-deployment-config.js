...
assert.doesNotMatch(dockerfile, /npm ci[^\n]*--force/, 'the release build must not force npm resolution');
assert.doesNotMatch(dockerfile, /chmod\s+777/, 'the release build must not create world-writable source');
assert.match(
