...
Record completed sessions as dated comments on [web #273](https://github.com/ontrack-features-t2-2026/doubtfire-web/pull/273), linking [API #173](https://github.com/ontrack-features-t2-2026/doubtfire-api/pull/173) and sanitised evidence. Copy the session/decision fields below and fill them with actual observations. Do not submit your own PR code approval.
...
2. Plan/Gantt: weekends, today, bars, hover/selection/focus and narrow scrolling. Export the synthetic plan to PNG and read every label. The export now uses light tokens during capture and restores the current appearance afterward. Verify readable task labels and unchanged saved preference; the downloaded PNG includes the full chart while browser print may preserve viewport clipping.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1  | Reuse CSV/XLS/XLSX API cases and the recorded XLSX browser submission; verify the persisted chair requirement in the student dialog. Additional browser formats are optional regression expansion | Stable `csv` category; original workbook bytes/worksheets retained; PDF carries download notice; configured task count and limit respected                                                                                                        |
...
Validated release candidate:
  web application SHA: ebd97e4a25a6de9c484b873ccb14559995c21f07
  browser-tested SHA: 64d6acd65fe264b1519f1b77610c17433af72f7b (later change is attribute order only)
  API SHA: 7722612ef019b03bd6b4fb9319b83c52b0dac902
  deploy: historical boundary run at8038777 with API6ca598ce; default1g, tested32m probe. No new deployment claimed
Organisation release branch: 11.0.x
Current web review path: #250 -> #251 -> #273; #273 targets fix/a11y-keyboard-controls-20260920
API #173 target: 11.0.x
Upstream repository/branch: [maintainer records the decision, or explicitly none for this release]
Integration/t2-2026 context: #192 is closed; its earlier demo-base proposal is not an approval to replace the recorded 11.0.x release path
Fallback/rollback branch or last known-good release:
Current competing branch/PR disposition: #250/#251 reused as ancestors, not duplicated; checked #272 and #274 combine cleanly at recorded heads. Further incoming changes need normal review.
Existing contributor work preserved: MG-04/MG-05, M02, upload policy/F01/DOCX/security and the accessibility keyboard work; see the dated attribution index
...
MVP includes: Light/Dark/System with existing account sync; readable core role journeys; task Spreadsheet CSV/XLS/XLSX; approved chat DOCX/CSV/XLSX plus existing media; server validation, authorised download and retained originals
Deferred with owner and issue: [no new deferral is approved by this template; record each actual decision]
Future work outside this upload scope: arbitrary files, active Office editing/preview, cloud-drive attachments and a new malware scanner
...
| FILE-T02   | Original implementation merged; follow-up PRs submitted        | [Delivered pack](UPLOAD-REGRESSION.md) | Physical clipboard/actual reader observations remain unrun | Pending reviewer record | Regression pack delivered; executed scope PASS, limitations retained | Maple Fox: review the recorded scope  |
| FILE-MVP01 | Original implementation merged; follow-ups #273/#173 submitted |                                        | Connected demo, risks, human decisions                     |                         | In progress until final evidence and decision                        | Maple Fox + named leadership reviewer |
...
| FILE-T02   | Review the delivered [combined regression pack](UPLOAD-REGRESSION.md); record any additional manual observation without repeating covered API/proxy cases                                                                         | Maple Fox for the scoped review; existing test evidence retained                       | Acceptance-to-test matrix, current/historical source labels, executed results and explicit unrun observations                                                                           |
