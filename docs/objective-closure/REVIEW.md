...
## Maple Fox's practical checklist

- [ ] Read [the results and open gaps](RESULTS.md); note the tested build and any fixes or limitations requiring review.
- [ ] Complete the [student-facing session](#student-facing-session), recording whether the appearance control, file requirements and submission history are understandable without help.
- [ ] Complete the [staff-facing session](#staff-facing-session), recording marking, viewer and unit-chair observations separately from the student role.
- [ ] Complete or explicitly record the remaining [theme/browser/accessibility checks](#theme-manual-matrix-and-exact-acceptance-checks) and [upload scenarios](#file-t02-connected-regression-and-file-mvp01-demo). Link evidence; leave unavailable checks BLOCKED or NOT RUN.
- [ ] Fill the [leadership and branch decision](#dated-leadership-and-branch-decision-template), including actual reviewer names, accepted limitations, risks, ownership and GO / CONDITIONAL GO / NO-GO. A code-review approval covers only its stated scope.
- [ ] Reconcile the [ticket closure map](#ticket-closure-map) using the [status rules](#status-rules-and-final-closure-record). Check off completion only where execution, evidence and required decisions are all recorded.
...
Cross the required route list in `THEME-REVIEW-AND-REGRESSION.md` with Light, Dark, System-light and System-dark; 1440×900 and 390×844; 100% and 200% zoom; Chromium, Firefox and Safari where available. Add 320px coverage if existing accessibility acceptance includes it. Do not claim the full cross-product from a subset.
...
4. Link MG-04 and MG-05 catalogues/style evidence if supplied; if unavailable, name the missing artifact and owner/action. Do not write a duplicate catalogue or claim external artifacts do not exist because the checkout lacks them.
...
MG-04 and MG-05 links / missing-artifact action:
...
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| THM-D01    | Reproduce raw and deduplicated audit at final SHA; classify high-impact findings; attach Light baseline or identify dated existing baseline; reconcile every required route; link MG-04/MG-05 work or record that its artifact was unavailable | Audit executor; Maple Fox reviews scope; existing MG owners retain attribution         | Exact command/output, counts, route rows, screenshot index, blocker/action table and MG links. Do not call candidate counts defect counts                                               |
