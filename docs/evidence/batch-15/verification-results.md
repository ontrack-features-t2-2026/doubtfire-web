...
| Web production bundle: `npm run build -- --configuration production`                        | **BLOCKED by runner resources**                                                                                                                                  | Host Node 26 aborted at optimisation with exit 134; the project Node 22 container was OOM-killed with exit 137 at `Building…`. Neither produced a TypeScript/compiler diagnostic. The container was restarted and restored on port 4400. This repeats Batch 01's resource boundary and is not a production-build pass.                        |
...
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 01    | Full web suite 119 files / 782 tests; typecheck/lint/development build pass; production optimiser OOM boundary documented                                                                   |
...
3. a production-optimised build on a memory-sufficient Node 22 release runner;
4. external SMTP arrival, because the available runtime has no outbound relay.
