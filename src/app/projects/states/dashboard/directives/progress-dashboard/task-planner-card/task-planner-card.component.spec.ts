...
describe('TaskPlannerCardComponent', () => {
  ...
  it('stacks the planner controls with space for the floating grade label', () => {
    ...
    const excludeCompleted = controls.querySelector('mat-checkbox');
    ...
    expect(gradeField?.nextElementSibling).toBe(excludeCompleted);
    expect(excludeCompleted?.nextElementSibling).toBe(downloadButton);
  });
