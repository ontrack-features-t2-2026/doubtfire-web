...
describe('ProjectPlanComponent', () => {
  ...
  it('groups target grade and calendar access as clear primary controls', async () => {
    ...
    const controls: HTMLElement = fixture.nativeElement.querySelector(
      '[aria-label="Task planner controls"]',
    );
    ...
    expect(calendarButton.textContent).toContain('Subscribe to calendar');

    calendarButton.click();
    expect(calendarModal.show).toHaveBeenCalledWith(null);
  });

  it('lets the planner grow naturally so phone task cards are not clipped', async () => {
    await createComponent(buildProject('Student', 25));

    const surface = fixture.nativeElement.querySelector('.task-planner-surface') as HTMLElement;
    expect(surface.style.height).toBe('');
    expect(surface.querySelector('f-task-planner')).not.toBeNull();
  });
