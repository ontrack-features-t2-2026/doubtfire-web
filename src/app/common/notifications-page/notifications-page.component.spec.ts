...
describe('NotificationsPageComponent', () => {
  ...
  it('says something friendly when there is nothing to show', () => {
    ...
    // empty placeholder, but a naive text-only assertion above could still be
    // fooled by stray markup. The list branch never renders this element.
    expect(fixture.nativeElement.querySelector('.notifications-empty')).not.toBeNull();
  });

  it('does not render the empty state while notifications are loading or the load failed', () => {
    fixture.detectChanges();

    expect(fixture.nativeElement.querySelector('.notifications-empty')).toBeNull();
  ...
  describe('before anyone is signed in', () => {
    ...
    it('asks nothing of the api when authentication settles after the page is left', () => {
      let resume: (signedIn: boolean) => void;
      authenticationService.afterAuthCall.mockImplementation((callback) => {
        resume = callback;
      });
