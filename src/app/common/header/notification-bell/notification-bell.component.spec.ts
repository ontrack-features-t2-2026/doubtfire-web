...
describe('NotificationBellComponent', () => {
  ...
  describe('the dropdown', () => {
    ...
    it('reads the unread count again every time it is opened', () => {
      const before = notificationService.refreshUnreadCount.mock.calls.length;

      openMenu();

      expect(notificationService.refreshUnreadCount).toHaveBeenCalledTimes(before + 1);
    });

    it('does not ask for the count when opened before authentication is ready', () => {
      authenticationService.isAuthenticated.mockReturnValue(false);
      const before = notificationService.refreshUnreadCount.mock.calls.length;

      openMenu();

      expect(notificationService.refreshUnreadCount).toHaveBeenCalledTimes(before);
    });
