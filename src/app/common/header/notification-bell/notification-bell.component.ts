 ...
 * an empty string, so anything added there is untested by construction.
 *
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  onMenuOpened(): void {
    ...
    // The badge is otherwise only read again on a navigation. A notification
    // that arrived while the user stayed on one page would show here as an
    // unread row under a badge that says nothing is waiting.
    this.refresh();
  ...
  open(notification: Notification): void {
    ...
    // Where it goes is decided in NotificationOpenService, which checks the
    // link before the router sees it.
