...
describe('NotificationRouteService', () => {
  ...
  it('accepts the url-encoded task codes the api sends', () => {
    const approved = [
      '/projects/23/dashboard/Task%201',
      '/projects/23/dashboard/Task%201/feedback',
      '/projects/23/dashboard/Pass%20Task%201.1',
      `/projects/23/dashboard/${'A'.repeat(128)}`,
      `/projects/23/dashboard/${'A'.repeat(128)}/feedback`,
    ];
  ...
  it('uses the safe fallback for missing and malformed values', () => {
    const invalid: unknown[] = [
      ...
      '/projects/1/dashboard/Task 1',
      `/projects/1/dashboard/${'A'.repeat(129)}`,
      `/projects/1/dashboard/${'A'.repeat(300)}`,
    ];
  ...
  it('rejects every escape except an encoded space inside a task code', () => {
    const invalid = [
      '/%2f%2fexample.test',
      '/%5cexample.test',
      '/projects/1/dashboard/%31.1P',
      '/projects/1/dashboard/%20Task',
      '/projects/1/dashboard/1.1P%3ftoken%3dsecret',
      '/projects/1/dashboard/1.1P%23feedback',
      '/projects/1/dashboard/1.1P%2Ffeedback',
      '/projects/1/dashboard/1.1P%5Cfeedback',
      '/projects/1/dashboard/1.1P%2e%2e',
      '/%252f%252fexample.test',
      '/projects/1/dashboard/%00',
      '/projects/1/dashboard/%',
      '/projects/1/dashboard/Task%2',
      '/projects/1%20/dashboard',
      '/notifications%20',
    ];
  ...
  it('navigates to an encoded task code as the api sent it', async () => {
    await service.navigate('/projects/23/dashboard/Task%201/feedback');

    expect(router.navigateByUrl).toHaveBeenCalledWith('/projects/23/dashboard/Task%201/feedback');
  });
  ...
  it('does not navigate twice when the target is already open', async () => {
