 ...
 *
 * Provided in root rather than declared in doubtfire-angular.module.ts, so the
 * notification pieces that follow can each use it without editing the same
 * providers array. Being a root singleton is also why reset() exists, see the
 * note there.
 */
...
export class NotificationService extends CachedEntityService<Notification> {
   ...
   * shared machine the next person to sign in sees both.
   *
